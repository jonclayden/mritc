...
for (m in c("EM", "ICM", "HMRFEM", "PVHMRFEM", "MCMC", "MCMCsub")) {
    out <- mritc(intarr, mask, method=m)
...
outdef <- mritc(intarr, mask)
expect_equal(outdef$method, "EM")

# method can be abbreviated
outabbr <- mritc(intarr, mask, method="IC")
